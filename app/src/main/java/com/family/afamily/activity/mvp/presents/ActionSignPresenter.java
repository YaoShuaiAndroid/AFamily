...
public class ActionSignPresenter extends BasePresent<PageSuccessView> {

    public ActionSignPresenter(PageSuccessView view) {
        attach(view);
    }

    public void getData(String token, String id, int page, final int getType,
                        final List<Map<String, String>> list, final SuperRecyclerView recyclerView, final ActionSignAdapter adapter) {
        if (getType == 1) {
            view.showProgress(3);
        }
        ...
        OkHttpClientManager.postAsyn(UrlUtils.ACTION_BM_INFO_URL, new OkHttpClientManager.ResultCallback<ResponsePageBean<Map<String, String>>>() {
            @Override
