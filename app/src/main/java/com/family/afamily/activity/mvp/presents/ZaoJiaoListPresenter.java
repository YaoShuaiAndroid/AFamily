...
public class ZaoJiaoListPresenter extends BasePresent<ZaoJaoListView> {
    public ZaoJiaoListPresenter(ZaoJaoListView view) {
        attach(view);
    }
    ...
    public void getDataList(String token, int p, String id, final int getType, final List<Map<String, String>> list, final SuperRecyclerView recyclerView, final ZaoJiaoListAdapter adapter) {
        if (getType == 1) {
            view.showProgress(3);
        }
        Map<String, String> params = new HashMap<>();
        ...
        params.put("type", id);
        params = Utils.getParams(params);
        OkHttpClientManager.postAsyn(UrlUtils.ZJ_VIDEO_LIST_URL, new OkHttpClientManager.ResultCallback<ResponsePageBean<Map<String, String>>>() {
            @Override
            public void onError(Request request, Exception e) {
                view.hideProgress();
                if (getType == 1) {
