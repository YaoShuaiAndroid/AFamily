...
public class MyExtensionPresenter extends BasePresent<PageSuccessView> {
    ...
    public MyExtensionPresenter(PageSuccessView view) {
        attach(view);
    }

    public void getData(String token, int type, int page, final int getType,
                        final List<Map<String, String>> list, final SuperRecyclerView recyclerView, final MyExtensionAdapter adapter) {
        if (getType == 1) {
            view.showProgress(3);
        }
        ...
        OkHttpClientManager.postAsyn(UrlUtils.EXT_LIST_URL, new OkHttpClientManager.ResultCallback<ResponsePageBean<Map<String, String>>>() {
            @Override
