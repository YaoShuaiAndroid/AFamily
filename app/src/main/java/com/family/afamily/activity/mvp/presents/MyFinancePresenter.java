...
public class MyFinancePresenter extends BasePresent<PageSuccessView> {

    public MyFinancePresenter(PageSuccessView view) {
        attach(view);
    }

    public void getData(String token, int page, final int getType,
                        final List<Map<String, String>> list, final SuperRecyclerView recyclerView, final MyFinanceAdapter adapter) {
        if (getType == 1) {
            view.showProgress(3);
        }
        ...
        OkHttpClientManager.postAsyn(UrlUtils.MY_FINANCE_LIST_URL, new OkHttpClientManager.ResultCallback<ResponsePageBean<Map<String, String>>>() {
            @Override
