...
public class IntegralAreaListPresenter extends BasePresent<PageSuccessView> {
    public IntegralAreaListPresenter(PageSuccessView view) {
        attach(view);
    }

    public void getData(String token, String cat_id, int page, final int getType,
                        final List<Map<String, String>> list, final SuperRecyclerView recyclerView, final IntegralAreaListAdapter adapter) {
        if (getType == 1) {
            view.showProgress(3);
        }
        ...
        OkHttpClientManager.postAsyn(UrlUtils.EXCHANGE_LIST_URL, new OkHttpClientManager.ResultCallback<ResponsePageBean<Map<String, String>>>() {
            @Override
