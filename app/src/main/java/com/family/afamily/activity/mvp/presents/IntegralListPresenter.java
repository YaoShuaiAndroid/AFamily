...
public class IntegralListPresenter extends BasePresent<PageSuccessView> {

    public IntegralListPresenter(PageSuccessView view) {
        attach(view);
    }

    public void getData(String token, String type, int page, final int getType,
                        final List<Map<String, String>> list, final SuperRecyclerView recyclerView, final IntegralListAdapter adapter) {
        if (getType == 1) {
            view.showProgress(3);
        }
        ...
        OkHttpClientManager.postAsyn(UrlUtils.INTEGRAL_LIST_LOG_URL, new OkHttpClientManager.ResultCallback<ResponsePageBean<Map<String, String>>>() {
            @Override
