...
public class FriendRankingPresenter extends BasePresent<PageSuccessView> {
    public FriendRankingPresenter(PageSuccessView view) {
        attach(view);
    }

    public void getData(String token, int page, final int getType,
                        final List<Map<String, String>> list, final SuperRecyclerView recyclerView, final FriendRankingAdapter adapter) {
        if (getType == 1) {
            view.showProgress(3);
        }
        ...
        OkHttpClientManager.postAsyn(UrlUtils.STUDY_PERCENT_LIST_URL, new OkHttpClientManager.ResultCallback<ResponsePageBean<Map<String, String>>>() {
            @Override
