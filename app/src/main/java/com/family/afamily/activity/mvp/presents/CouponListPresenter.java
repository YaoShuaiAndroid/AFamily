...
public class CouponListPresenter extends BasePresent<PageSuccessView> {

    public CouponListPresenter(PageSuccessView view) {
        attach(view);
    }

    public void getData(String token, final int type, int page, final int getType,
                        final List<Map<String, String>> list, final SuperRecyclerView recyclerView, final CouponListAdapter adapter) {
        if (getType == 1) {
            view.showProgress(3);
        }
        ...
        OkHttpClientManager.postAsyn(UrlUtils.MY_COUPON_URL, new OkHttpClientManager.ResultCallback<ResponsePageBean<Map<String, String>>>() {
            @Override
