...
import com.family.afamily.activity.mvp.presents.OpenMemberPresenter;
...
public class OpenMemberActivity extends BaseActivity<OpenMemberPresenter> implements OpenMemberView {
    @BindView(R.id.open_vip_money)
    TextView openVipMoney;
    @BindView(R.id.open_vip_content)
    TextView openVipContent;
    ...
    @Override
    public OpenMemberPresenter initPresenter() {
    ...
    public void initDataAsync() {
        ...
        btnSubmit.setOnClickListener(new View.OnClickListener() {
            ...
            public void onClick(View v) {
                ...
                presenter.submitData(token, payType);
//                if(isBalance){
//                }else{
//                    String payType = payWxCheck.isChecked() ? "wxpay" : "alipay";
//                    presenter.submitData(token,payType);
//                }
            }
        });
    ...
    public void successData(Map<String, String> data) {
        if (data != null) {
            openVipContent.setText(Html.fromHtml(data.get("content")));
            openVipMoney.setText("¥" + data.get("year_price") + "/年");
            Double year_price = Double.parseDouble(data.get("year_price"));
            Double user_money = Double.parseDouble(data.get("user_money"));

            if (user_money >= year_price) {
                isBalance = true;
                payBalanceItem.setVisibility(View.VISIBLE);
                payWxCheck.setChecked(false);
                payBalanceCheck.setChecked(true);
            } else {
                isBalance = false;
                payBalanceItem.setVisibility(View.GONE);
                payBalanceCheck.setChecked(false);
                payWxCheck.setChecked(true);
            }
        }
