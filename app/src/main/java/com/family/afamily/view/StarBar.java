...
public class StarBar extends View {
     ...
     * @param mark
     */
    public void setStarMark(float mark, int type) {
        if (integerMark) {
        ...
        if (this.onStarChangeListener != null) {
            this.onStarChangeListener.onStarChange(starMark, type);  //调用监听接口
        }
        invalidate();
    }
    ...
    public interface OnStarChangeListener {
        void onStarChange(float mark, int type);
    }
        ...
        if (isTouch) {
            ...
            switch (event.getAction()) {
                case MotionEvent.ACTION_DOWN: {
                    setStarMark(x * 1.0f / (getMeasuredWidth() * 1.0f / starCount), 1);
                    break;
                }
